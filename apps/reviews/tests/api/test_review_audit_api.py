...
class ReviewAuditAPITest(BaseReviewTest, APITestCase):
    def test_author_can_view_audit(self):
        ...
        assert results[0]["action"] in {ReviewAuditAction.CREATED, ReviewAuditAction.EDITED}
        assert any(item["action"] == ReviewAuditAction.CREATED for item in results)
    ...
    def test_reason_is_saved_in_audit(self):
        ...
        assert item["action"] == ReviewAuditAction.MODERATOR_REMOVED
